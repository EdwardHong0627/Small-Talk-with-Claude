...
pub async fn list_reactions(
    ...
    let conn = state.conn();
    let mut stmt =
        conn.prepare("SELECT kind, COUNT(*) FROM reactions WHERE slug = ?1 GROUP BY kind")?;
    ...
    Ok(Json(Value::Object(counts)).into_response())
}
...
pub async fn create_reaction(
    ...
    // `COUNT(*)` always returns exactly one row, so this can never hit the
    // "no rows" case that `unwrap_or` used to guard against.
    ...
    let mut stmt =
        conn.prepare("SELECT kind, COUNT(*) FROM reactions WHERE slug = ?1 GROUP BY kind")?;
    let rows = stmt.query_map([payload.slug.trim()], |row| {
        let kind: String = row.get(0)?;
        let count: i64 = row.get(1)?;
        Ok((kind, count))
    })?;
    ...
    Ok((StatusCode::OK, Json(Value::Object(counts))).into_response())
}

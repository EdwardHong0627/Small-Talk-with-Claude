...
/// slugs, oldest first.
pub async fn pending_comments(State(state): State<AppState>) -> Response {
...
pub async fn approve_comment(State(state): State<AppState>, Path(id): Path<i64>) -> Response {
    let conn = state.conn.lock().unwrap();
    let updated = conn
        .execute(
...
/// approved).
pub async fn delete_comment(State(state): State<AppState>, Path(id): Path<i64>) -> Response {
    let conn = state.conn.lock().unwrap();
    let deleted = conn
...
pub async fn list_contact(State(state): State<AppState>) -> Response {
    let conn = state.conn.lock().unwrap();
    let mut stmt = conn
        .prepare(
